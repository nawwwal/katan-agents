...
The game needs a title theme, a low match bed, and a victory resolve. Try the
ElevenLabs Music API (`/v1/music`) first — it is priced differently from sound
effects and may be blocked on free tier.

**Probe with the shortest generation the endpoint allows before planning around
it.** If music is unavailable or costs more than the remaining budget allows,
say so plainly and fall back to the looping sound-effects model for a sparse
ambient bed. Do not silently spend the SFX budget on music, and do not ship a
thin generated loop as a "theme" — an honest ambience bed beats a bad theme.

| id | intent |
|----|--------|
| `music-title` | seafaring orchestral, warm strings, light hand percussion, hopeful, loops |
| `music-match` | subdued low bed, minimal melody, must survive hours of play without irritating |
| `music-victory` | short triumphant resolve, ~8s, does not loop |
